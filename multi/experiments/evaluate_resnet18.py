"""
ResNet18 双分支分类评估脚本 - 用于消融实验对比
python -m multi.experiments.evaluate_resnet18 --checkpoint checkpoints/resnet18_dual_branch_best.pt --split test
"""
import os
import sys
import yaml
...
class ResNet18Evaluator:
    ...
    def evaluate(self, data_loader: DataLoader, debug: bool = False) -> dict:
        ...
        for batch_idx, batch_data in enumerate(eval_bar):
            ...
            # 预测
            deception_result, suppression_result = self.model.predict(stft_images)

            pred_deception_idx = deception_result['indices'][:, 0]
            ...
            all_labels_deception.append(labels_deception)
            all_labels_suppression.append(labels_suppression)
            ...
            if debug and not debug_done:
                ...
                for i in range(min(5, batch_size)):
                    ...
                    pred_deception = [self.deception_classes_with_none[pred_deception_idx[i].item()]]
                    pred_suppression = [self.suppression_classes_with_none[pred_suppression_idx[i].item()]]
            ...
            for i in range(batch_size):
                if pred_deception_idx[i] == labels_deception_idx[i] and pred_suppression_idx[i] == labels_suppression_idx[i]:
        ...
        # 计算指标
        deception_metrics = self._compute_metrics(all_labels_deception, all_preds_deception)
        suppression_metrics = self._compute_metrics(all_labels_suppression, all_preds_suppression)
    ...
    def print_metrics(self, metrics: dict):
        """打印评估指标"""
        print("\n" + "=" * 80)
        print("ResNet18 Dual-Branch Evaluation Results")
        print("=" * 80)
    ...
    def plot_confusion_matrices(self, metrics: dict, save_path: str = None):
        ...
        if labels_deception is not None:
            ...
            sns.heatmap(cm, annot=True, fmt='d', cmap='Blues',
                        ...
                        ax=axes[0])
            axes[0].set_title('欺骗分支混淆矩阵 (Deception Branch)')
        ...
        if labels_suppression is not None:
            ...
            sns.heatmap(cm, annot=True, fmt='d', cmap='Greens',
                        ...
                        ax=axes[1])
            axes[1].set_title('压制分支混淆矩阵 (Suppression Branch)')
    ...
    def plot_metrics_summary(self, metrics: dict, save_path: str = None):
        ...
        bars1 = axes[0].bar(x, deception_values, width=0.6, color='steelblue')
        axes[0].set_ylabel('Score')
        axes[0].set_title('欺骗分支指标 (Deception Branch)')
        axes[0].set_xticks(x)
        ...
        bars2 = axes[1].bar(x, suppression_values, width=0.6, color='seagreen')
        axes[1].set_ylabel('Score')
        axes[1].set_title('压制分支指标 (Suppression Branch)')
        axes[1].set_xticks(x)
    ...
    def plot_combination_confusion_matrix(
        ...
        ax.set_title('ResNet18 组合混淆矩阵 - [S]=已见(Seen), [U]=未见(Unseen)')
    ...
    def print_combination_metrics(
        ...
        for i in range(len(labels_deception)):
            true_comb = get_combination_indices(labels_deception[i], labels_suppression[i])
            pred_comb = get_combination_indices(preds_deception[i], preds_suppression[i])
    ...
    def save_stft_predictions(
        ...
        for batch_data in tqdm(data_loader, desc="Collecting"):
            (stft_images, _, _,
             labels_deception, labels_suppression, _, _,
             metadata_list) = batch_data
        ...
        for sample in tqdm(selected_samples, desc="Saving STFT images"):
            ...
            # 预测
            deception_result, suppression_result = self.model.predict(image)
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate ResNet18 Dual-Branch Model")
    parser.add_argument("--config", type=str, default="multi/config.yaml")
    parser.add_argument("--checkpoint", type=str, required=True)
    parser.add_argument("--split", type=str, default="test", choices=["train", "val", "test"])
    parser.add_argument("--output_dir", type=str, default="results/resnet18")
    ...
    results_path = output_dir / f"resnet18_results_{args.split}.json"
    ...
    evaluator.plot_confusion_matrices(
        metrics,
        save_path=str(output_dir / f"resnet18_confusion_{args.split}.png")
    )

    # 每个类别的 F1 分数
    evaluator.plot_per_class_f1(
        metrics,
        save_path=str(output_dir / f"resnet18_per_class_f1_{args.split}.png")
    )

    # 指标汇总
    evaluator.plot_metrics_summary(
        metrics,
        save_path=str(output_dir / f"resnet18_metrics_summary_{args.split}.png")
    )

    # 组合混淆矩阵
    evaluator.plot_combination_confusion_matrix(
        metrics,
        save_path=str(output_dir / f"resnet18_combination_confusion_{args.split}.png"),
